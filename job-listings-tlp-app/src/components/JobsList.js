...
export default function JobsList(props) {
    ...
    return (
        <div className={'jobs-list'}>
            <FilterBar filters={languageFilters.concat(toolFilters)} removeFilter={removeFilter} clearFilters={clearFilters} />
            {jobs.map((job) => <JobCard key={job.id} job={job} toggleLanguageFilter={toggleToolFilter} toggleToolFilter={toggleToolFilter} />)}
        </div>
    );
}